...
## О проекте
- Микросервис, предназначенный для работы с балансом пользователей.
- База данных - postgres.
- Фреймворк для Rest API - gorilla.
- Данные запросов проходят валидацию.
- Этот проект был для меня отличным шансом впервые попробовать свои силы в программировании микросервисов и в Go.
В ходе работы над ним я разобралась в основах микросервисной архитектуры, как работать с контейнеризацией для разработки и добавила в копилку новый язык программирования.  
...
6. Доп. задания:
    - Сценарий разрезервирования денег в случае отмены заказа
    - Сценарий для получения истории последних операций над балансом пользователя, обрабатываются заказы и переводы от пользователя к пользователю
